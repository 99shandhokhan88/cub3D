...
void	move_forward(t_game *game, double move_speed)
{
	...
	new_pos_x = (game->posX + game->dirX * move_speed);
	new_pos_y = (game->posY + game->dirY * move_speed);
		...
		&& new_pos_y >= 0 && new_pos_y < game->map->width)
	{
		if (game->map->grid[(int)new_pos_x][(int)new_pos_y] == 0
			&& game->map->grid[(int)game->posX][(int)game->posY] == 0)
		{
			game->posX = new_pos_x;
			game->posY = new_pos_y;
		}
...
void	move_left(t_game *game, double move_speed)
{
	...
	new_pos_x = game->posX + (-game->dirY) * move_speed;
	new_pos_y = game->posY + (game->dirX) * move_speed;
	if (new_pos_x >= 0 && new_pos_x < game->map->height
		&& new_pos_y >= 0 && new_pos_y < game->map->width)
	{
		if (game->map->grid[(int)new_pos_x][(int)new_pos_y] == 0
