...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/08 00:39:30 by vzashev           #+#    #+#             */
/*   Updated: 2025/01/01 22:25:58 by vzashev          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rotate_player(t_params *params, double rot_speed)
{
	...
	params->dirX = params->dirX * cos(rot_speed)
		- params->dirY * sin(rot_speed);
	params->dirY = old_dir_x * sin(rot_speed)
		+ params->dirY * cos(rot_speed);
	params->planeX = params->planeX * cos(rot_speed)
		- params->planeY * sin(rot_speed);
	params->planeY = old_plane_x * sin(rot_speed)
		+ params->planeY * cos(rot_speed);
}
		...
		double move_speed, double new_pos_x, double new_pos_y)
{
	...
	if (new_pos_x >= 0 && new_pos_x < params->game->map->width
		&& new_pos_y >= 0
		&& new_pos_y < params->game->map->height)
	{
		...
		double move_speed, double new_pos_x, double new_pos_y)
{
	...
	new_pos_y = (params->posY - params->dirY * move_speed) + 0.0042;
	if (new_pos_x >= 0 && new_pos_x < params->game->map->width
		&& new_pos_y >= 0 && new_pos_y < params->game->map->height)
	{
		check_x1 = (int)new_pos_x;
		check_y1 = (int)params->posY;
		check_x2 = (int)params->posX;
